...
Spots go to their own `spots` collection, never mixed with ride logs: a Route 4 spotted at one stop and another Route 4 spotted at the next are two buses, not a timed hop. They do appear on the Track page and on trip cards ("seen from the stop"), and they feed the punctuality chart on the private stats page — each one an arrival at a stop, which is what punctuality modelling needs.
...
- **Picking a ride back up.** If no fix has matched the road for 45 seconds — the app was closed, the screen locked, GPS lost — the bus has kept driving, so the last known position is stale. The follower then searches the whole rest of the route for where the phone is now, needs a second fix to agree, skips the stops passed in the meantime (no report, no invented time: *"Found the bus again — 3 stops passed while tracking was paused"*) and carries on logging. "Got off" is not judged until the bus is found; if three fixes find no road at all, the rider most likely got off while tracking was away, and the usual check takes over. Before this, reopening the app mid-ride was always read as getting off.
