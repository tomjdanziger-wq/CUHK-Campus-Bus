...
(function (root) {
  ...
  function runsToday(route, date) {
    return matchesDay(route.runsOn, date);
  }
  ...
  function serviceWindow(route, date) {
    ...
    for (var i = 0; i < overrides.length; i++) {
      if (!matchesDay(overrides[i].runsOn, date)) continue;
      if (overrides[i].firstDeparture) first = parseHHMM(overrides[i].firstDeparture);
      if (overrides[i].lastDeparture) last = parseHHMM(overrides[i].lastDeparture);
      break;
    }
