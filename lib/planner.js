...
(function (root) {
      ...
      for (var i = 0; i < route.stops.length - 1; i++) {
        ...
        for (var j = i + 1; j < route.stops.length; j++) {
          ...
          options.push({
            ...
            onwardStops: onwardFrom(route, j, stopsById, fromStop, cfg, boardId),
   ...
   */
  function onwardFrom(route, fromIndex, stopsById, fromStop, cfg, boardId) {
    var out = [];
    for (var k = fromIndex + 1; k < route.stops.length && out.length < 4; k++) {
      var stop = stopsById[route.stops[k]];
      if (!stop) continue;
      // A loop brings you back past where you boarded; offering that as
      // somewhere to get off is noise. Any other stop is fair game — including
      // a loop's last stop, which is often the station everyone wants.
      if (route.stops[k] === boardId) continue;
  ...
  function compareOptions(a, b) {
    ...
    // Same stop, same arrival: prefer the one that walks you less at the far
    // end. Without this, Route H from Wu Yee Sun dropped you at S.H. Ho for a
    // seven-minute walk downhill to the station, because that stop came first
    // in the list — when staying on to the station door arrived no later.
    var aAfter = a.kind === 'walk' ? a.walk.minutes : a.walkFromStop.minutes;
    var bAfter = b.kind === 'walk' ? b.walk.minutes : b.walkFromStop.minutes;
    return aAfter - bAfter;
  }

  function routeIdsOf(o) {
    return (o.routes || [o.route]).map(function (r) { return r.id; });
  }

  /** Do these two options put you on any of the same buses? */
  function sharesRoute(a, b) {
    var ids = routeIdsOf(b);
    return routeIdsOf(a).some(function (id) { return ids.indexOf(id) !== -1; });
  }
  ...
  function selectShuttleOptions(all, directWalk, cfg) {
    ...
    var useful = all.filter(function (o) {
      ...
      // Sanity cap, on time spent MOVING only. It used to include the wait,
      // which made buses vanish the moment one pulled away: at 10:06 Route 4
      // from Wu Yee Sun to the station was gone, because its next departure
      // was twenty minutes off — though it was still the bus to the door.
      // A route stays on the list with its next departure; the ranking and the
      // wait line say how long that is. Only a ride that is slower than
      // walking even with no wait at all is dropped here.
      if (o.travelMinutes > directWalk.minutes + cfg.search.maxWorseThanWalkMinutes) return false;
    ...
    // Best option per leg — a leg being a boarding stop and an alighting stop.
    //
    // Deliberately NOT one per boarding stop. From the downhill stop at Wu Yee
    // Sun, Route 4 runs to the station door while routes 3, 7 and 6A stop at
    // the Piazza three minutes' walk short. Standing at the same kerb, those
    // are two real choices, and collapsing them threw away the one most people
    // would wait an extra couple of minutes for.
    //
    // Keyed by the routes too. Two buses on the same leg whose ride times are
    // too different to group are different journeys: from Wu Yee Sun to the
    // station, Route 4 goes straight down while Route 8 loops the west side.
    // Keyed by leg alone, whichever left sooner won — so for the twenty
    // minutes after a Route 4 pulled away, Route 8 took its slot, was then
    // dropped as a duplicate of another Route 8 option, and Route 4 was gone.
    var bestByLeg = {};
    grouped.forEach(function (o) {
      var k = o.boardStop.id + '>' + o.alightStop.id + '|' + routeIdsOf(o).join('+');
      if (!bestByLeg[k] || compareOptions(o, bestByLeg[k]) < 0) bestByLeg[k] = o;
    });

    var legs = Object.keys(bestByLeg).map(function (k) { return bestByLeg[k]; });

    // Two options from the same stop that share a route are the same bus —
    // the only difference is where you get off, and "Or stay on for" already
    // covers that. Keep the best and skip the rest.
    //
    // Done greedily, best first, rather than by dropping every option some
    // rival beats. That version quietly lost Route 4 from Wu Yee Sun: the
    // slower "3, 4, 7 or 6A to S.H. Ho" beat "4 to the station", and was
    // itself beaten by "3, 7 or 6A to the Piazza", so both fell and the only
    // bus going to the station door disappeared from the list.
    legs.sort(compareOptions);

    var keptPerStop = {};
    var candidates = [];
    legs.forEach(function (o) {
      var kept = keptPerStop[o.boardStop.id] || (keptPerStop[o.boardStop.id] = []);
      if (kept.some(function (k) { return sharesRoute(o, k); })) return;
      kept.push(o);
      candidates.push(o);
    });

    // The same bus offered from two nearby stops, arriving at the same time,
    // is one option presented twice. Keep the first (best) of them.
    //
    // Only when they really do arrive together. This used to drop ANY later
    // option on the same route, so Route 8 from the downhill stop to Science
    // Centre silently removed Route 8 from the uphill stop to the station
    // door — a different bus, in the other direction, to a different place.
    var seenRoute = {};
    candidates = candidates.filter(function (o) {
      var ids = routeIdsOf(o).join('+');
      var prev = seenRoute[ids] || (seenRoute[ids] = []);
      if (prev.some(function (p) {
        return Math.abs(o.arrivalMinutes - p.arrivalMinutes) <= TIE_EPSILON_MINUTES;
      })) return false;
      prev.push(o);
      return true;
    });
    ...
    flagLongWayRound(chosen, cfg);

    // Fill the slots with sensible options before wrong-side ones.
    //
    // Without this, a flagged option can crowd out an unflagged one purely by
    // being a few minutes quicker — so the list ends up showing the bus that
    // goes the wrong way round while hiding the one that goes to the door.
    // Nothing is lost by preferring the clean ones: an option is only flagged
    // when some unflagged bus already beats it, so the best option is never
    // the flagged one.
    var cap = cfg.search.maxOptions - 1;                    // room for the walk
    var trimmed = chosen.filter(function (o) { return !o.longWayRound; }).slice(0, cap);
    chosen.filter(function (o) { return o.longWayRound; }).forEach(function (o) {
      if (trimmed.length < cap) trimmed.push(o);
    });

    trimmed.sort(compareOptions);
    return trimmed;
  }
   ...
   * the entire circuit.
   *
   * The comparison is TIME SPENT RIDING, not total time. That distinction is
   * the whole thing. Measured on total, this flagged Route 4 from Wu Yee Sun —
   * the direct bus to the station door, boarding on the correct kerb — purely
   * because its next departure was fifteen minutes off. Telling someone they
   * are on the wrong side of the road when they have simply just missed the
   * bus is worse than saying nothing. On the real cases the penalty shows up
   * as roughly eight extra minutes of riding; on a missed bus the ride is the
   * same length or shorter and every lost minute is in the wait.
   *
  ...
  function flagLongWayRound(options, cfg) {
    ...
    buses.forEach(function (o) {
      // The shortest ride available from somewhere this rider could just as
      // easily be standing. A stop genuinely further away being slower is not
      // a mistake, it is just further away.
      var rival = null;
      buses.forEach(function (other) {
        if (other === o) return;
        if (other.travelMinutes >= o.travelMinutes) return;
        if (rival && other.travelMinutes >= rival.travelMinutes) return;
        if (geo.walk(o.boardStop, other.boardStop, cfg).minutes >
            cfg.search.longWayRoundWalkMinutes) return;
        rival = other;
      });
      if (!rival) return;

      var penalty = o.travelMinutes - rival.travelMinutes;
      ...
      o.longWayRound = {
        ...
        sameStopPair: !!(o.boardStop.pair && o.boardStop.pair === rival.boardStop.pair)
      };
    });
  }
  ...
  function plan(origin, destination, data, now) {
    ...
    if (!picked.length) {
      // Two different reasons, and the user should hear the true one. "The bus
      // barely saves any walking" and "the bus would help, but not for another
      // twenty minutes" call for different decisions: in the second case
      // someone who would rather not walk can simply wait.
      var helpful = raw.filter(function (o) {
        return walkOpt.walk.minutes - (o.walkToStop.minutes + o.walkFromStop.minutes) >=
               cfg.search.minWalkSavedMinutes;
      }).sort(function (a, b) { return a.arrivalMinutes - b.arrivalMinutes; });

      noShuttleReason = helpful.length
        ? 'The next useful shuttle is ' + helpful[0].route.name + ' at ' +
          helpful[0].departureTime + ' from ' + helpful[0].boardStop.name +
          ', arriving around ' + helpful[0].arrivalTime + '. Walking gets you there well before that.'
        : raw.length
  ...
  /** The final departure from a route's first stop on this date, or null. */
  function lastDepartureAt(route, date) {
    var w = serviceWindow(route, date);
    if (w.first === null || w.last === null) return null;
    var best = null;
    for (var hour = Math.floor(w.first / 60); hour <= Math.floor(w.last / 60); hour++) {
      (w.minutes || []).forEach(function (m) {
        var t = hour * 60 + m;
        if (t >= w.first && t <= w.last && (best === null || t > best)) best = t;
      });
    }
    return best;
  }
  ...
  /**
   * The last bus of the day on every route that gets you near `destination`.
   *
   * This answers a different question from plan(). plan() asks "what should I
   * do now?"; this asks "how late can I leave it?" — which, at eleven at night
   * with the Night Service about to stop, is the only question that matters.
   *
   * With an `origin`, each route is boarded at whichever nearby stop gives
   * the quickest trip, and the time is the last departure FROM THAT STOP.
   * Without one there is nowhere to board from, so the time is the last
   * departure from the route's first stop, and routes that start next to the
   * destination anyway are left out.
   *
   * Every time here is scheduled, like everywhere else in the app. A last bus
   * is the one most worth being early for.
   *
   * Returns rides sorted latest first, at most one per route.
   */
  function lastRides(destination, data, date, origin) {
    var cfg = data.config;
    var lastCfg = cfg.lastBus || { maxWalkFromStopMinutes: cfg.search.maxWalkFromStopMinutes };
    var stopsById = {};
    data.stops.forEach(function (s) { stopsById[s.id] = s; });

    var home = {}, reach = {};
    data.stops.forEach(function (s) {
      home[s.id] = geo.walk(s, destination, cfg);
      if (origin) reach[s.id] = geo.walk(origin, s, cfg);
    });
    var direct = origin ? geo.walk(origin, destination, cfg).minutes : Infinity;
    var nowMinutes = minutesSinceMidnight(date);

    var out = [];
    data.routes.forEach(function (route) {
      if (!runsToday(route, date)) return;
      var last = lastDepartureAt(route, date);
      if (last === null) return;

      var best = null;
      for (var j = 1; j < route.stops.length; j++) {
        var walkHome = home[route.stops[j]];
        // Tighter than the planner's limit. "The last bus home" that leaves
        // you a quarter of an hour's walk away at midnight is not one.
        if (!walkHome || walkHome.minutes > lastCfg.maxWalkFromStopMinutes) continue;

        for (var i = 0; i < (origin ? j : 1); i++) {
          var board = stopsById[route.stops[i]];
          if (!board) continue;
          var walkTo = origin ? reach[board.id] : null;

          if (origin) {
            if (walkTo.minutes > cfg.search.maxWalkToStopMinutes) continue;
            // Same test as the planner: the bus has to save real walking.
            if (walkTo.minutes + walkHome.minutes > direct - cfg.search.minWalkSavedMinutes) continue;
          } else if (home[board.id].minutes <= walkHome.minutes + cfg.search.minWalkSavedMinutes) {
            continue;
          }

          var ride = geo.ride(route, i, j, stopsById, cfg);
          var cost = (walkTo ? walkTo.minutes : 0) + ride.minutes + walkHome.minutes;
          if (best && best.cost <= cost) continue;

          var boardAt = last + (i > 0 ? geo.ride(route, 0, i, stopsById, cfg).minutes : 0);
          best = {
            cost: cost,
            route: route,
            boardStop: board,
            alightStop: stopsById[route.stops[j]],
            boardIndex: i,
            alightIndex: j,
            boardMinutes: boardAt,
            boardTime: formatHHMM(boardAt),
            arriveMinutes: boardAt + ride.minutes + walkHome.minutes,
            arriveTime: formatHHMM(boardAt + ride.minutes + walkHome.minutes),
            walkToStop: walkTo,
            walkHome: walkHome,
            fromFirstStop: !origin,
            // Gone if you could not reach the stop in time, walking from here.
            gone: nowMinutes + (walkTo ? walkTo.minutes : 0) > boardAt
          };
        }
      }
    ...
    out.sort(function (a, b) { return b.boardMinutes - a.boardMinutes; });
    return out;
  }
