 ...
 *                                also "ERB 407", room number ignored)
 *   prefix match          800
...
(function (root) {
   ...
   */
  /**
   * "ERB 407" → "erb", "LSK LT6" → "lsk", "YIA LT 8" → "yia".
   *
   * A calendar entry is a building code followed by a room. Nobody's building
   * is called "407", so drop the trailing room tokens — anything with a digit,
   * or a lecture-theatre / floor marker — and match what is left. Returns null
   * when there is nothing to strip.
   */
  function withoutRoom(q) {
    // Typed without the space: "erb407".
    var glued = /^([a-z]{2,4})(\d{3,}[a-z]?)$/.exec(q);
    if (glued) return glued[1];

    var words = q.split(' ');
    var n = words.length;
    while (n > 1 && /\d|^(lt|rm|room|g|lg|ug|f)$/.test(words[n - 1])) n--;
    return n < words.length ? words.slice(0, n).join(' ') : null;
  }

  function search(query, places, limit) {
    var q = normalise(query);
    if (!q) return [];
    var qBuilding = withoutRoom(q);
    ...
    for (var i = 0; i < places.length; i++) {
      ...
      if (p.aliases) {
        for (var a = 0; a < p.aliases.length; a++) {
          ...
          // Only an exact code counts once the room is stripped: "erb 407"
          // should find ERB, not every building with an "erb" somewhere.
          if (qBuilding) {
            var sb = scoreField(qBuilding, p.aliases[a], true);
            if (sb >= 950 && sb > s) s = sb;
          }
      ...
      // Words spread across the name and the aliases: "fusion grocery" is
      // Fusion Supermarket (name) + grocery (alias), "canteen new asia" a
      // New Asia canteen. Every word must start some word somewhere on the
      // place; ranked below a match within one field.
      if (!best) {
        var qWords = q.split(' ').filter(Boolean);
        if (qWords.length > 1) {
          var words = [];
          [p.name].concat(p.aliases || []).forEach(function (f) {
            words = words.concat(normalise(f).split(' '));
          });
          var all = qWords.every(function (w) {
            return words.some(function (x) { return x.indexOf(w) === 0; });
          });
          if (all) { best = 600; matched = p.name; }
        }
      }
      if (!best) continue;
