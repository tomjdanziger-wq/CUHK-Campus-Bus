...
const ADMINS = (globalThis.FIREBASE_ADMIN_UIDS || []).filter((u) => /^[A-Za-z0-9]{10,64}$/.test(u));
// How far back anyone may read. The app only asks for the last showMinutes.
const PUBLIC_READ_HOURS = Math.ceil(DATA.config.tracking.showMinutes / 60) + 1;
...
//
// Rider reports for the CUHK Shuttle Helper. Anyone may read the last few
// hours of them; only the admin accounts may read the history. A phone
...
// how the real stop-to-stop times get measured (see stats.html).

service cloud.firestore {
  match /databases/{database}/documents {

    // The private stats page: these account ids may read everything.
    function isAdmin() {
      return request.auth != null && request.auth.uid in [${ADMINS.map((u) => `'${u}'`).join(', ')}];
    }

    // Everyone else, the app included, reads only the last ${PUBLIC_READ_HOURS} hours —
    // enough for "reported by riders", not the history.
    function isRecent(data) {
      return data.t > request.time - duration.value(${PUBLIC_READ_HOURS}, 'h');
    }
...
${map}
    ...
    match /sightings/{id} {
      allow get: if isAdmin() || isRecent(resource.data);
      allow list: if isAdmin() || (request.query.limit <= 200 && isRecent(resource.data));
